...
) BEGIN DECLARE v_dias_atraso INT;
...
SELECT
    ...
    data_devolucao INTO v_data_emprestimo,
    v_data_prevista,
    v_data_devolucao
FROM
    EMPRESTIMO
WHERE
    id_emprestimo = p_id_emprestimo;

-- Determinar a data final do período a ser considerado
SET
    v_data_fim_periodo = COALESCE(v_data_devolucao, CURDATE());

-- Calcular dias de atraso
SET
    v_dias_atraso = DATEDIFF(v_data_fim_periodo, v_data_prevista);
...
ELSE -- Buscar o valor da multa que estava vigente durante o período de atraso
SELECT
    valor_por_dia INTO v_valor_dia
FROM
    PARAMETRO_MULTA
WHERE
...
ORDER BY
    ...
    CASE
        WHEN v_data_prevista BETWEEN data_inicio_vigencia
        AND COALESCE(data_fim_vigencia, v_data_prevista) THEN 1
        ELSE 2
    END,
...
-- Valor padrão caso não encontre parâmetro
END IF;

-- Calcular multa
SET
    p_valor_multa = v_dias_atraso * v_valor_dia;
...
SELECT
    @valor_multa AS 'Valor da Multa Empréstimo 1';